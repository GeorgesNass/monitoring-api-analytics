...
#   - build metrics only (BigQuery views/tables or SQLite dashboard tables)
#   - run FastAPI service
#   - run tests (pytest)
###############################################################################
...
while true; do
  ...
  echo " 1) Run full pipeline (extract -> transform -> load -> metrics)"
  echo " 2) Build metrics only (deploy SQL assets / materialize dashboard tables)"
